...
function main()
	...
	ASHelperMessage('Введите /swat чтобы открыть настройки.')
	checkstyle()
	imgui.Process = false
										...
										for DialogLine in text:gmatch('[^\r\n]+') do
											...
											if tonumber(zakonstatusint) then
												if tonumber(zakonstatusint) >= 35 then
													if not text:find('Леичися в псифхиатрической больыницееееее') then
														if not text:find('Состоит в ЧС{FF6200} Полиция ЛВ') then
															...
															passvalue = false
															passverdict = ('в чс сват')
														end
													end
												else
													passvalue = false
													passverdict = ('незаконопослушный')
												end
											end
										end
									else
...
if imguicheck and encodingcheck then
	...
	local Ranks_arr 					= {u8'[1] Стажёр',u8'[2] Практикант',u8'[3] Сержант',u8'[4] Мл. Оперативник',u8'[5] Оперативник',u8'[6] Офицер первого звена',u8'[7] Лейтенант',u8'[8] Командор',u8'[9] Зам.Директора'}
	
	local sobesdecline_select 			= imgui.ImInt(0)
	local sobesdecline_arr 				= {u8'Проф.непригоден'}
	...
	local buttons 						= {fa.ICON_FA_USER_COG..u8' Настройки пользователя',fa.ICON_FA_FILE_ALT..u8' Тренировки',fa.ICON_FA_KEYBOARD..u8' Горячие клавиши',fa.ICON_FA_PALETTE..u8' Настройки цветов',fa.ICON_FA_BOOK_OPEN..u8' Правила, шпоры',fa.ICON_FA_INFO_CIRCLE..u8' Информация о скрипте'}
				...
				if imgui.Button(u8'Отклонить', imgui.ImVec2(285,30)) then
					if not inprocess then
						if not mcvalue or not passvalue then
							if mcverdict == ('наркозависимость') or mcverdict == ('не полностью здоровый') or passverdict == ('меньше 3 лет в штате') or passverdict == ('не законопослушный') or passverdict == ('игрок в организации') or passverdict == ('был в деморгане') or passverdict == ('в чс полиция лв') or passverdict == ('есть варны') then
								windows.imgui_sobes.v = false
								...
								elseif passverdict == ('в чс полиция лв') then
									sampSendChat('К сожалению, я не могу продолжить собеседование. Вы находитесь в ЧС SWAT.')
	...
	• Команды: /swat - настройки хелпера, /swatbind - биндер хелпера, /swatrek - меню лекций.
	
	• Настройки: Введя команду /swat, откроются настройки, в которых можно изменять никнейм в приветствии, акцент, создание маркера при выделении, пол, горячую клавишу быстрого меню и другое.

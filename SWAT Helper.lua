...
function main()
	...
	getmyrank = true
	ASHelperMessage(('SWAT Helper %s успешно загружен. Автор: JustMini'):format(thisScript().version))
										...
										for DialogLine in text:gmatch('[^\r\n]+') do
											...
											if tonumber(zakonstatusint) then
												if tonumber(zakonstatusint) >= 35 then
													if not text:find('Леичился в псифхиатрической больыницееееее') then
...
if imguicheck and encodingcheck then
	...
	local buttons 						= {fa.ICON_FA_USER_COG..u8' Настройки пользователя',fa.ICON_FA_FILE_ALT..u8' хуй слона',fa.ICON_FA_FILE_ALT..u8' хуй слона№2',fa.ICON_FA_KEYBOARD..u8' Горячие клавиши',fa.ICON_FA_PALETTE..u8' Настройки цветов',fa.ICON_FA_BOOK_OPEN..u8' Правила автошколы',fa.ICON_FA_INFO_CIRCLE..u8' Информация о скрипте'}
	...
	• Меню лекций: Введя команду /ashlect откроется меню лекций, в котором вы сможете озвучить/добавить/удалить лекции.]]
			...
			if settingswindow == 2 then
				imgui.TextColoredRGB('ХУЙ СЛОНА {808080}(?)',1)
				imgui.Hint('тут хуй')
...
if settingswindow == 6 then
				imgui.TextColoredRGB('Вы може1те добавлять свои правила!{808080} (?)',1)
				imgui.Hint('Вы долж1ны создать .txt файл с кодировкой ANSI\nЛКМ для открытия папки с правилами')
				if imgui.IsMouseReleased(0) and imgui.IsItemHovered() then
					createDirectory(getWorkingDirectory()..'\\SWAT Helper\\Rules')
					os.execute('explorer '..getWorkingDirectory()..'\\SWAT Helper\\Rules')
				end
...
function checkbibl()
	...
	downloadUrlToFile('https://raw.githubusercontent.com/ozr1236/swat/main/updateswathelperrr.ini', 'moonloader/updateswathelperr.ini', function(id, status)
		if status == dlstatus.STATUSEX_ENDDOWNLOAD then
			if doesFileExist('moonloader/updateswathelperr.ini') then
